...
from cond_xla import conditional_xla

def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, use_hvd=False, optimizer_type="adam", use_fp16=False, manual_fp16=False):
...
class AdamWeightDecayOptimizer(tf.compat.v1.train.Optimizer):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    for (grad, param) in grads_and_vars:
      ...
      has_shadow = self.manual_fp16 and param.dtype.base_dtype != tf.float32
      if has_shadow:
      ...
      next_param, next_m, next_v = apply_adam_with_decay(self.beta_1, 
        self.beta_2, self.learning_rate, self.epsilon, 
        self._do_use_weight_decay(param_name),
        self.weight_decay_rate, grad, m, v, param)
...
@conditional_xla()
def apply_lamb(beta_1, beta_2, learning_rate, epsilon, do_decay, weight_decay_rate, grad, m, v, param):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    for (grad, param) in grads_and_vars:
      ...
      v = tf.compat.v1.get_variable(
          ...
          trainable=False,
          initializer=tf.compat.v1.zeros_initializer())
      next_param, next_m, next_v = apply_lamb(self.beta_1, self.beta_2,
